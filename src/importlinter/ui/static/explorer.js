...
function buildApiUrl(moduleName) {
    ...
    if (showIndirect) {
        params.push('show_indirect=true');
    }
    if (params.length) {
        url += '?' + params.join('&');
    }
...
function buildCacheKey(moduleName) {
    return `${moduleName}|${showImportTotals}|${showCycleBreakers}|${depth}|${showIndirect}`;
}
...
function onSettingsChange() {
    ...
    showIndirect = document.getElementById('toggle-indirect').checked;
